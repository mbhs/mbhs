	...
	return (
		<div className="pb-10 dark:text-white">
			...
			<div className="flex flex-col gap-3 px-5 sm:px-8 md:px-10 lg:px-16 xl:px-24">
				{achievements.map(({ attributes: { title, description, image, link, publishOn } }, i) => (
                    <div>
                        {/* Compares the displayed date with what the previous one was and hides it if they are the same */}
                        <Markdown className={
                            new Date(achievements[i-(i > 0 ? 1 : 0)].attributes.publishOn).toLocaleString("default", {
                                timeZone: "UTC",
                                month: "long",
                                year: "numeric"
                            }) !== new Date(publishOn).toLocaleString("default", {
                                timeZone: "UTC",
                                month: "long",
                                year: "numeric"
                            }) || i === 0 ? "visible" : "hidden"
