			...
			>
				...
				<div className="hidden -mt-1 lg:flex flex-col">
					<div className="flex flex-row lg:gap-2">
						...
						<motion.button
							onClick={() => {
								setDropdownOpen((prev) => !prev),
									quickLinkButton.current?.focus();
							}}
							onBlur={() => {
								setDropdownOpen(false), quickLinkButton.current?.blur();
							}}
							ref={quickLinkButton}
							initial={{ opacity: 0 }}
							animate={{ opacity: 1 }}
						...
						{(process.env.NO_I18N !== "1") && <Link className="px-2 py-auto rounded-lg bg-black bg-opacity-20 hover:bg-opacity-25 text-white" href={router.asPath} locale={locale === "en" ? "es" : "en"}>
							{locale === "en" ? "es" : "en"}
						</Link>}
					</div>
				</div>
