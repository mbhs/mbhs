	...
	links: [
		...
		},
		{
			...
			attributes: {
				...
				link: "/resources",
				links: [],
				image: { data: { attributes: { url: "no picture" } } },
			},
		},
		{
			id: 6,
...
function ImageDropdownNavLink({
	...
	toggle,
	animate,
}: ImageDropdownNavLinkProps) {
	//should there be an option to not have a dropdown?
	return (
		<div>
			<Link
				onClick={() => toggle("all", false)}
				onMouseOver={() => toggle(name, true)}
				...
				<motion.div
					...
					onMouseEnter={() =>
						toggle(
							name,
							true &&
								+document.getElementById(name + "NavLink")!.style.opacity > 0.75
						)
					}
					onMouseLeave={() => toggle(name, false)}
				...
				>
					...
					<div className="flex flex-col p-4 gap-3">
						{links.map(({ attributes: { text, link } }) => (
							<Link href={link} key={text + "DropdownLink"} className="text-lg">
								{text}
							</Link>
						))}
					</div>
		...
		<div>
			<div className="flex mx-auto justify-between text-white text-xl">
				<div className="w-full rounded-lg">
					<Link
						onClick={() => {
							sideBar(false), (document.body.style.overflow = "");
						}}
						href={link}
					>
						{name}
					...
					{Object.keys(links).length > 0 && (
						<motion.div
							className="pl-6 bg-red-800 w-full rounded-md"
							initial={animate.exit}
							animate={isClick[name] ? "enter" : "exit"}
							variants={animate}
						>
							<ul className="py-2 space-y-2">
								{links.map(({ attributes: { text, link } }) => (
									<li key={text + "SideBarLink"}>
										<Link
											href={link}
											onClick={() => {
												sideBar(false), (document.body.style.overflow = "");
						...
						{examplePull.links.map(
							({ attributes: { name, link, links, image } }, prop) => (
								<ImageDropdownNavLink
									...
									links={links}
									isHover={linkSelected}
									toggle={toggleLinkSelected}
